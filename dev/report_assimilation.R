...
errorStatsFilePath <- file.path(evalDirPath, sprintf("errorStats_assimilation_%s.csv", errorMetric))
...
for (i in seq_len(nrow(experiments))) {
  ...
  plt <-
    experiErrorStats |>
    filter(mean == min(mean), .by = c(noiseScale, methodName)) |>
